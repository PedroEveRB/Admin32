...
export const StreamingForm: React.FC = () => {
  ...
  const loadStreaming = async () => {
    try {
      ...
      setFormData({
        ...
        senha_stream: '', // Não carregamos a senha por segurança
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!id && !formData.senha_stream) {
        throw new Error('Senha Stream é obrigatória para nova streaming');
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            />
            <Input
              label={id ? "Nova Senha Stream (deixe vazio para manter)" : "Senha Stream (FTP/Conexão) *"}
              name="senha_stream"
              type="password"
