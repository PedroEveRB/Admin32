...
router.post('/', authenticateToken, requireLevel(['super_admin', 'admin']), async (req, res) => {
  try {
    const {
      codigo_cliente, plano_id, codigo_servidor, usuario, senha, senha_stream, identificacao, email,
      espectadores, bitrate, espaco, aplicacao, idioma_painel, descricao
    } = req.body;

    if (!senha_stream) {
      return res.status(400).json({ message: 'Senha Stream é obrigatória' });
    }
    ...
    const senhaTransmissaoHash = await bcrypt.hash(senha_stream, 10); // Mesma senha para transmissão por padrão
...
router.put('/:id', authenticateToken, requireLevel(['super_admin', 'admin']), async (req, res) => {
  try {
    ...
    if (senha_stream && senha_stream.trim() !== '') {
      const senhaStreamHash = await bcrypt.hash(senha_stream, 10);
      updateQuery += ', senha_transmissao = ?';
      params.push(senhaStreamHash);
    }
